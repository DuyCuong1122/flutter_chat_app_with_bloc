...
import 'package:chat_app/page/chat_box/widget/chat_list.dart';
...
import '../../bloc/message_chat/message_chat_event.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => MessageChatBloc(message: message)
        ..add(MessageGetAllChatEvent(message: message)),
      child: GestureDetector(
        onTap: () => FocusScope.of(context).unfocus(),
        child: Scaffold(
          backgroundColor: AppColors.f6Color,
          appBar: buildAppBar(heightScreen, context, message, userId),
          body: Container(
            ...
            child: Column(
              children: [
                BlocBuilder<MessageChatBloc, MessageChatState>(
                    builder: (context, state) {
                  if (state is MessageChatSuccessState) {
                    return ChatList(
                      msgContentList: state.messagesList,
                      message: message,
                    );
                  }
...
AppBar buildAppBar(
    double heightScreen, BuildContext context, Message message, String userId) {
  return AppBar(
